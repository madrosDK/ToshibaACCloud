...
class ToshibaAC extends IPSModule
{
    ...
    public function GetStatus()
    {
        ...
        if (!$result || empty($result['ResObj'])) {
            echo "❌ Keine Statusdaten erhalten.\n";
            return;
        }
        ...
        if (!empty($state['ACStateData'])) {
            $decoded = $this->DecodeACStateData($state['ACStateData']);
    ...
    public function GetConfigurationForm()
    {
        ...
        foreach ($devices as $device) {
            $options[] = ['caption' => "{$device['name']} ({$device['id']})", 'value' => $device['id']];
        }
    ...
    private function Login($username, $password)
    {
        ...
        if (!$result || empty($result['ResObj']['access_token']) || empty($result['ResObj']['consumerId'])) {
            $this->DebugLog(__FUNCTION__, 'Login fehlgeschlagen oder unvollständig');
            return false;
        }
    ...
    private function QueryAPI($url, $postData = null, $token = null)
    {
        ...
        if ($result === false || $result === '') {
            $this->DebugLog(__FUNCTION__, 'Fehler bei Query: ' . $url . ' / ' . $error);
            return false;
        }
